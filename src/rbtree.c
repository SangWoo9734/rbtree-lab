...
int tree_to_array_inorder(const rbtree *t, key_t *arr, size_t n, int *index) {
  if ( *index >= n) {
    return *index;
  }
  ...
  if ( t -> root -> left != t -> nil ) {
    ...
    tree_to_array_inorder( &lt, arr, n, index );
  }
 ...
 // 트리의 오른쪽 순회
  if ( t -> root -> right != t -> nil ) {
    ...
    tree_to_array_inorder( &rt, arr, n, index );
  }
...
int rbtree_to_array(const rbtree *t, key_t *arr, const size_t n) {
 ...
 return tree_to_array_inorder(t, arr, n, &index);
}